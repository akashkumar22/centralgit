...
#Find Prod-HTTPS-ASG
data azurerm_application_security_group "Prod-HTTPS-ASG" {
  name                = "Prod-HTTPS-ASG"
  resource_group_name = "NetworkSecurityGroups-RG"
}
#Find Prod-Windows-ASG
data azurerm_application_security_group "Prod-Windows-ASG" {
  name                = "Prod-Windows-ASG"
  resource_group_name = "NetworkSecurityGroups-RG"
}
#Find Prod-AutoAny-ASG
data azurerm_application_security_group "Prod-AutoAny-ASG" {
  name                = "Prod-AutoAny-ASG"
  resource_group_name = "NetworkSecurityGroups-RG"
}
#Associate ASGs to server NICs
resource azurerm_network_interface_application_security_group_association "Prod-HTTPS" {
  depends_on = [module.VMserver]
  application_security_group_id = data.azurerm_application_security_group.Prod-HTTPS-ASG.id
  network_interface_id          = module.NIC.vm_nic_id
}
resource azurerm_network_interface_application_security_group_association "Prod-Windows" {
  depends_on = [module.VMserver]
  application_security_group_id = data.azurerm_application_security_group.Prod-Windows-ASG.id
  network_interface_id          = module.NIC.vm_nic_id
}
resource azurerm_network_interface_application_security_group_association "Prod-AutoAny-ASG" {
  depends_on = [module.VMserver]
  application_security_group_id = data.azurerm_application_security_group.Prod-AutoAny-ASG.id
  network_interface_id          = module.NIC.vm_nic_id
}
#Find Prod-NSG
data azurerm_network_security_group "Prod-NSG" {
  name                = "Prod-NSG"
  resource_group_name = "NetworkSecurityGroups-RG"
}
#Associcate server NICs to the NSG
resource azurerm_network_interface_security_group_association "Prod-NSG" {
  depends_on = [module.VMserver]
#  for_each = toset(var.VMserver_name)
  network_interface_id      = module.NIC.vm_nic_id
  network_security_group_id = data.azurerm_network_security_group.Prod-NSG.id
}
